...
def vouchers(request):
    ...
    if request.method == 'POST':
        rq.delete('http://127.0.0.1:8084/vouchers/' + request.POST['voucherId'])

    vouchers_all = rq.get('http://127.0.0.1:8084/vouchers').json()
...
def add_voucher(request):
    ...
    if request.method == 'POST':
        result = rq.post(
            url='http://127.0.0.1:8084/vouchers',
            data=json.dumps(request.POST),
            headers={'Content-Type': 'application/json'}
        )
...
def edit_voucher(request, id):
    ...
    if request.method == 'POST':
        result = rq.put(
            url='http://127.0.0.1:8084/vouchers/' + str(id),
            data=json.dumps(request.POST),
            headers={'Content-Type': 'application/json'}
        )
    ...
    voucher = rq.get('http://127.0.0.1:8084/vouchers/' + str(id)).json()
    voucher['id_label'] = f'#{voucher["voucherId"]:08X}'
    print(voucher)
